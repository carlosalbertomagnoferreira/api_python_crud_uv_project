...
async def main():
    async with httpx.AsyncClient() as client:
        ...
        assert response.status_code == HTTPStatus.BAD_REQUEST, (
            'Email duplicado não retornou erro'
        )
        ...
        response2 = await client.post(
            f'{BASE_URL}/users', params={'name': 'Maria', 'email': 'maria@example.com'}
        )
        ...
        assert response.status_code == HTTPStatus.BAD_REQUEST
        print(
            'Erro corretamente detectado ao tentar atualizar para email duplicado:',
            response.json(),
        )
